...
export class FocusTrapAction {
  ...
  set = <K extends keyof FocusTrapState>(key: K, value: FocusTrapState[K]) => {
  ...
  onFocusTrap = (root: Document, getTabbable: FocusTrapProps['getTabbable']) => {
    ...
    const onFocusIn = (e: FocusEvent) => {
      ...
      const { $start, $content, $end, isShiftTab, lastFocus } = this.states
      ...
      if (active !== $start.current && active !== $end.current) {
        if (container.contains(target)) return this.set('lastFocus', target)

        if (lastFocus) return this.focusElement(lastFocus)
      }
