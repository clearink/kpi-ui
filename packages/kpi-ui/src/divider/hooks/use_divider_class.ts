...
export default function useDividerClass(name: string, props: DividerProps) {
  const {
    type, dashed, orientation, children, plain, className, orientationMargin,
  } = props;

  // 自定义边距
  const customMargin = useMemo(() => ['left', 'right'].includes(orientation) && orientationMargin !== undefined, [orientation, orientationMargin]);
  return useMemo(() => cls(
    name,
    {
      [`${name}--${type}`]: type,
